...
func (m *Manager) SetModelResolver(resolver func(pub_models.Chat) string) {
	if resolver == nil {
		return
	}
...
func (m *Manager) resolveModelPrice(ctx context.Context) (ModelPriceScheme, error) {
	...
	if m.fetcher == nil {
		return ModelPriceScheme{}, fmt.Errorf("resolve model price for %q: missing model catalog fetcher", m.model)
	}
...
func (m *Manager) Enrich(chat pub_models.Chat) (pub_models.Chat, error) {
	...
	if m.modelResolver != nil {
		if resolved := m.modelResolver(chat); resolved != "" {
			modelName = resolved
		}
	}
