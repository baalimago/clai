...
type sessionFinalizerer interface {
	Finalize(context.Context, *QuerySession) error
}
...
func (r *sessionRunner[C]) Run(ctx context.Context, session *QuerySession) (runErr error) {
	...
	defer func() {
		...
		// A failed reply persist joins the run's returned error (S8) so a
		// later -re/-dre cannot silently read stale state; the finalizer runs
		// last, so the join sees every error the run produced.
		if ferr := r.finalizer.Finalize(ctx, session); ferr != nil {
			runErr = errors.Join(runErr, ferr)
			session.Failed = true
		}
	}()
	...
	for stepIndex := 0; ; {
		...
		stepResult, err := r.executeModelStep(ctx, session)
		if err != nil {
		...
		// Consumer-supplied usage recorders never abort the run: the consumer
		// owns the recorder and observes its own failures (worklog
		// 2026-09-05-error-propagation, S10 never-abort contract).
